 ...
 */
     ...
     * @param <V>
     */
    public class BPlusTree <T extends Comparable<T>, V> implements  java.io.Serializable{
        ...
        private ArrayList<LeafNode> datas = new ArrayList<LeafNode>();
        public ArrayList<String> dataKeys = new ArrayList<String>();
        public ArrayList<String> dataValues = new ArrayList<String>();
        ...
        class InternalNode<T extends Comparable<T>, V> extends Node<T, V> {
            ...
            private Node<T, V> insert(T key, Node<T, V> leftChild, Node<T, V> rightChild){
                ...
                if(this.size <= MAX_CHILDREN_FOR_INTERNAL) {
                    // 源数组 源数组起始位置 目的数组 目的数组起始位置 复制的长度
                    System.arraycopy(newKeys, 0, this.keys, 0, this.size);
                    //pointers的size 加1了，而key的size 没有加1

                    //System.out.println("newPointers.length   " + newPointers.length);
                    //System.out.println("size   " + size);
                    //System.out.println("this.pointers   " + this.pointers.length);
                    System.arraycopy(newPointers, 0, this.pointers, 0, this.size + 1);

                   // System.out.println("MAX_CHILDREN_FOR_INTERNAL   " + MAX_CHILDREN_FOR_INTERNAL);
                    return null;
                }
        ...
        class LeafNode<T extends Comparable<T>, V> extends Node<T, V> {
            ...
            public Node<T, V> insert(T key, V value) {
                ...
                for (; i < this.size; i++) {
                    ...
                    if (curKey.compareTo(key) == 0) {
                        this.values[i] = (String)this.values[i]+";" +value;
                        return null;
                    }
            ...
            public V get(T key) {
               ...
               // System.out.println("size-----" + size);
                ...
                while (start < end) {
                    ...
                    //System.out.println("middleKey-----" + middleKey);
            ...
            void getall() {
                ...
                for (int i = 0; i < this.size; i++) {
                    dataKeys.add((String) this.keys[i]);
                    dataValues.add((String)this.values[i]);
                }
            }
...
//
//            myTree.getall();
//            System.out.println(myTree.datas.size()*5);
//            for (int i = 0; i < max; i++) {
...
//
//
//                //System.out.println(myTree.datas.get(i).values[0].toString());
////            for (Object key : myTree.datas.get(i).keys) {
//                //System.out.println(key);
////				if(key != null &&(Integer)key % 5 == 0 ){
////					System.out.println(key);
////				}
////			}
//            }
//            for (int j = 0; j < myTree.datas.size(); j++) {
//                for (Object key : myTree.datas.get(j).keys) {
//
//                    if(key != null && key.equals(100) ){
//                        //	(String)key.contains(100);
...
//            }
//
//            for (Object key : myTree.datas.get(16).keys) {
//                System.out.println(key);
//            }
